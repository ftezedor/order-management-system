...
- Java 21+
- Maven 3.9+
- Docker (for Testcontainers, RabbitMQ and Zipkin)
...
Integration tests utilize Testcontainers for PostgreSQL and RabbitMQ. Ensure that your Docker daemon is running before launching test tasks.

If your tests require dynamic property assignment for the dynamic ports assigned by Testcontainers, make sure to resolve connection variables through Spring Boot's @DynamicPropertySource without deprecated methods:

```java
@Container
protected static final RabbitMQContainer rabbitmqContainer = 
    new RabbitMQContainer(DockerImageName.parse("rabbitmq:3.13-management"))
        .withAdminUser("test-rabbit-user")
        .withAdminPassword("test-rabbit-password");
```
